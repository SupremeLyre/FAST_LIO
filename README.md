...
### 3.1 For MID360
Connect to your PC to Livox MID360 LiDAR by following  [Livox-ros-driver installation](https://github.com/Livox-SDK/livox_ros_driver2), then
```
    cd ~/$FAST_LIO_ROS_DIR$
    source devel/setup.bash
...
```
- For livox serials, FAST-LIO only support the data collected by the ``` msg_MID360.launch ``` since only its ``` livox_ros_driver2/CustomMsg ``` data structure produces the timestamp of each LiDAR point which is very important for the motion undistortion. ``` rviz_MID360.launch ``` can not produce it right now.
- If you want to change the frame rate, please modify the **publish_freq** parameter in the [msg_MID360.launch](https://github.com/Livox-SDK/livox_ros_driver2/blob/master/launch_ROS1/msg_MID360.launch) of [Livox-ros-driver2](https://github.com/Livox-SDK/livox_ros_driver2) before make the livox_ros_driver2 pakage.
